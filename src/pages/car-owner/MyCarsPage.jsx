...
import { Car, Plus, X, ToggleLeft, ToggleRight, Users, MapPin, Camera } from "lucide-react";
import { validateLicensePlate } from "../../hooks/Valid";
...
const MyCarsPage = () => {
  ...
  const [imageModal, setImageModal] = useState(null); // carId for image management modal
  ...
  // Auto-open modal when ?add=1 is present in URL
  useEffect(() => {
  ...
  const load = () => {
    setL(true);
    axiosInstance.get(`/api/rental-cars/owner/${user.id}`)
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (!form.make) return toast.error("Car make is required");
    const plateCheck = validateLicensePlate(form.licensePlate);
    if (!plateCheck.valid) return toast.error(plateCheck.message);
    setSave(true);
    try {
      await axiosInstance.post(`/api/rental-cars/owner/${user.id}/list`, {
        make:            form.make,
        model:           form.model,
  ...
  return (
    <div className="page-container space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-gray-900">My Cars</h1>
          <p className="text-gray-500 text-sm mt-1">
            {cars.length} car{cars.length !== 1 ? "s" : ""} listed for rental
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          ...
          <p className="text-gray-400 text-xs mt-1">List your car to start earning</p>
          <button onClick={open} className="btn-primary mt-4 text-xs">List a Car</button>
        </div>
      ) : (
        <div className="space-y-3">
          {cars.map((car) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-500">
                      <span className="flex items-center gap-1"><Users size={10} /> {car.seatingCapacity}</span>
        ...
        <div className="fixed inset-0 bg-black/50 z-modal flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl w-full max-w-lg my-6">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                <h2 className="font-display text-lg font-bold text-gray-900">List Your Car</h2>
                <p className="text-gray-500 text-xs mt-0.5">Fill in the details to list your car for rental</p>
              </div>
              <button onClick={close} className="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center">
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleCreate} className="p-5 space-y-4">
              ...
              {/* Pickup Address with interactive map */}
              <div className="space-y-1.5">
                <label className="label flex items-center gap-1.5">
                  <MapPin size={12} className="text-green-600" />
                  Pickup Address
                  <span className="text-gray-400 font-normal text-xs ml-1">— type or click on map</span>
                </label>
                <PickupAddressPicker
      ...
      {imageModal && (
        <div className="fixed inset-0 bg-black/50 z-modal flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl w-full max-w-md my-6">
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                <h2 className="font-display text-lg font-bold text-gray-900">Car Photos</h2>
                <p className="text-gray-500 text-xs mt-0.5">
                  Upload up to 6 photos · JPG, PNG or WebP · Max 5MB each
                </p>
              </div>
              <button
                onClick={() => { setImageModal(null); load(); }}
