...
const ActiveJobPage = () => {
  ...
  // NEW: State for customer/car map coordinates
  const [customerLoc, setCustomerLoc] = useState(null);
  ...
  useEffect(() => {
    axiosInstance
      .get(`/api/valet/request/${requestId}`)
      .then((r) => {
        const currentStatus = normalise(r.data?.status);
        setStatus(currentStatus);
        setVType(r.data?.vehicleType || "CAR");
        if (r.data?.batteryLevelAtPickup  != null) setPickupBattery(r.data.batteryLevelAtPickup);
        if (r.data?.batteryLevelAtParking != null) setParkBattery(r.data.batteryLevelAtParking);

        // Extract coordinates to show the blue destination marker on the map!
        // If parked/returning, use parked location. Otherwise, use pickup location.
        if (["PARKED", "RETURN_CONFIRM_PENDING", "RETURN_REQUESTED"].includes(currentStatus)) {
          if (r.data?.parkedLatitude && r.data?.parkedLongitude) {
            setCustomerLoc({ lat: r.data.parkedLatitude, lng: r.data.parkedLongitude });
          } else if (r.data?.pickupLatitude && r.data?.pickupLongitude) {
            setCustomerLoc({ lat: r.data.pickupLatitude, lng: r.data.pickupLongitude }); // fallback
          }
        } else {
          if (r.data?.pickupLatitude && r.data?.pickupLongitude) {
            setCustomerLoc({ lat: r.data.pickupLatitude, lng: r.data.pickupLongitude });
          }
        }
      })
      .catch(() => toast.error("Could not load job status"))
  ...
  // NEW: Keep syncing the Valet's live GPS to the server while on this page!
  useEffect(() => {
    if (location && user?.id) {
      axiosInstance.post(`/api/valet/location`, {
        ...
        available: false // False because the valet is busy with an active job
      }).catch(() => console.log("Failed to sync live GPS"));
    }
  }, [location, user]);
