...
//     // Dynamic import keeps Leaflet out of the initial bundle if unused
//     import("leaflet").then((L) => {
//       // CRITICAL FIX: Prevent double initialization from async race conditions
//       if (!isMounted || mapRef.current || mapContainerRef.current._leaflet_id) return;
...
//     // Cleanup on unmount
//     return () => {
//       isMounted = false; // Prevent async init if unmounted before import finishes
//       if (mapRef.current) {
//         mapRef.current.remove();
//         mapRef.current = null;
//         valetMarkerRef.current = null;
...
//         // Ensure the Leaflet ID is cleared from the DOM node
...
const ValetTrackingMap = ({ mode, requestId, customerLocation, etaMinutes, status }) => {
  ...
  useEffect(() => {
    ...
    // FIXED: Expanded ACTIVE_STATUSES so the map polls your location through the ENTIRE job!
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      // Customer / Destination marker (blue)
      if (customerLocation) {
        const cLabel = mode === "valet" ? "📍 Destination" : "📍 Your location";
        const cMarker = L.marker(
          [customerLocation.lat, customerLocation.lng],
          { icon: makeDotIcon(L, "#3b82f6") }
        )
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      const { lat, lng } = valetLocation;
      const vLabel = mode === "valet" ? "🚗 You (Live)" : "🚗 Your valet";
      ...
      // NEW: Aggressive Auto-Focus. Always pan map to center the Live GPS location
      mapRef.current.panTo([lat, lng], { animate: true });
    });
  }, [valetLocation, mapReady, mode]);
  ...
  }, [customerLocation, mapReady, mode]);
    ...
    <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-0 bg-white border-t border-gray-100">
        <div className="flex flex-col items-center justify-center py-3 px-2 border-r border-gray-100">
          ...
          <span className="text-[10px] text-gray-400 uppercase tracking-wide">Status</span>
          <p className="text-xs font-semibold text-gray-900 mt-0.5 text-center leading-tight">
            {status ? status.replace(/_/g, " ") : <span className="text-gray-400">—</span>}
          </p>
        </div>
      ...
      {/* Legend - Updated dynamically based on who is using the map! */}
      <div className="flex items-center gap-4 bg-gray-50 px-4 py-2 border-t border-gray-100">
        <span className="flex items-center gap-1.5 text-[11px] text-gray-500">
          <span className="w-2.5 h-2.5 rounded-full bg-blue-500 inline-block" />
          {mode === "valet" ? "Destination" : "You"}
        </span>
        <span className="flex items-center gap-1.5 text-[11px] text-gray-500">
          <span className="w-2.5 h-2.5 rounded-full bg-orange-500 inline-block" />
          {mode === "valet" ? "You (Live)" : "Valet"}
        </span>
        <span className="ml-auto text-[10px] text-gray-400">Updates every 5s</span>
      </div>
    </div>
